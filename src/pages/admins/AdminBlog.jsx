...
import { normalizeForSave } from '../../utils/html';
...
export default function AdminBlog() {
    ...
    // view mode: 'form' or 'data' — controls whether form or data list is shown
    const [viewMode, setViewMode] = useState('data');
    // custom dropdown open state and ref for click-outside
    ...
    // preview and auto-upload on selection
    useEffect(() => {
        // if a local file object is set we show a local preview until upload completes
    ...
    // react-quilljs (quill v2) config — use a toolbar container that we'll position inside the editor
    ...
    // bind text-change once and cleanup
    useEffect(() => {
        if (!quill) return;
    ...
    // Ensure editor styling and toolbar are re-applied when the form becomes visible.
    useEffect(() => {
        if (!quill) return;
        if (viewMode !== 'form') return;
        // small delay to allow the ref DOM to mount
        const t = setTimeout(() => {
            try {
    ...
    // when editor or editingId changes, populate content and adjust sizing so it grows with content
    useEffect(() => {
        if (!quill) return;
    ...
    // show toolbar when editor is focused / selection present
    useEffect(() => {
        if (!quill) return;
    ...
    // Quill creates its own toolbar DOM; move it into our custom container inside editor
    useEffect(() => {
        if (!quill) return;
        try {
            const tbModule = quill.getModule && quill.getModule('toolbar');
            ...
            const container = document.getElementById(toolbarId);
            if (tbDom && container && container !== tbDom.parentElement) {
                // remove from current parent and append into our container
                tbDom.parentElement && tbDom.parentElement.removeChild(tbDom);
                container.appendChild(tbDom);
            }
        } catch (e) { /* ignore */ }
        // style the editor root to visually match other inputs and the provided design
        try {
            if (quill.root && quill.root.style) {
                quill.root.style.background = '#F0F7FF';
                quill.root.style.color = '#000000';
                // larger inner padding so placeholder and content have breathing room
                quill.root.style.padding = '18px 20px';
                quill.root.style.borderRadius = '12px';
                quill.root.style.minHeight = '220px';
                // subtle inset blue line + soft outer shadow to mimic the screenshot
                quill.root.style.boxShadow = 'inset 0 0 0 2px rgba(3,119,255,0.12), 0 4px 12px rgba(3,119,255,0.06)';
                quill.root.style.fontSize = '16px';
                quill.root.style.lineHeight = '1.5';
                quill.root.style.boxSizing = 'border-box';
        ...
        // remove default inner grey border/outline produced by Quill
        try {
            ...
            const editor = quill.root && quill.root.classList ? quill.root : null;
            if (editor) {
    ...
    // also inject a tiny stylesheet to ensure quill toolbar and editor match global input styles
    useEffect(() => {
        const id = 'admin-blog-quill-styles';
        if (document.getElementById(id)) return;
        const style = document.createElement('style');
        style.id = id;
        style.innerHTML = `
            /* make the quill editor look like other inputs used in the form */
            .ql-container { background: transparent !important; border: none !important; box-shadow: none !important; }
            /* match input padding: px-6 (24px) py-3 (12px) and visual style */
            ...
            /* placeholder styling to match input placeholder color and position */
            ...
            /* toolbar appearance and spacing inside the rounded input box */
            ...
            /* toolbar icons color to match primary */
            #${toolbarId} .ql-toolbar button, #${toolbarId} .ql-toolbar .ql-stroke { color: #0377FF; }
            /* normalize toolbar layout and center all icons (svg and text-based) */
            ...
            /* small nudge for certain SVGs that render slightly off-center */
            ...
            /* lists: align markers and avoid double bullets from Quill */
            ...
            .prose li::marker { font-size: 0.9em; vertical-align: middle; color: currentColor; }
    ...
    useEffect(() => {
        if (!quill) return;
        quill.clipboard.dangerouslyPasteHTML(form.content || '');
        // make editor larger and allow it to expand with content
        try {
            if (quill.root && quill.root.style) {
                quill.root.style.minHeight = '220px';
                quill.root.style.maxHeight = 'none';
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            } catch (e) { /* fallback to existing form.content */ }
            // sanitize
            ...
            // ensure content exists; if admin left it empty, preserve empty string
            payload.content = normalizeForSave(safeHtml || form.content || '');
            payload.meta_description = payload.meta_description || '';
            // if tags UI used, serialize tags to comma-separated meta_keyword
            if (form.tags && Array.isArray(form.tags)) {
                payload.meta_keyword = form.tags.join(',');
            } else {
                payload.meta_keyword = payload.meta_keyword || '';
            }
            try { console.log('DEBUG: admin sending blog content (delta->html->sanitized):', (payload && payload.content) ? String(payload.content).slice(0, 500) : '(no content)'); } catch (e) { }
            if (editingId) {
                await api.put(`/blogs/${editingId}`, payload);
                setEditingId(null);
                // set message only; show popup after reload via URL param
                setSuccessMsg('Blog berhasil diperbarui!');
                setErrorMsg('');
            } else {
                // New articles are created as unpublished by default
                payload.published = false;
                await api.post("/blogs", payload);
                // set message only; show popup after reload via URL param
                setSuccessMsg('Blog berhasil dibuat!');
                setErrorMsg('');
            }
            // close form, reset and reload to data view with success indicator
            try {
                const url = new URL(window.location.href);
                url.searchParams.delete('edit');
                url.searchParams.set('saved', '1');
                // include the content type name so popup reads e.g. "Blog — Berhasil diperbarui!"
                url.searchParams.set('msg', editingId ? 'Blog Berhasil diperbarui!' : 'Blog Berhasil dibuat!');
                window.location.href = url.pathname + '?' + url.searchParams.toString();
                return;
            } catch (e) {
                // redirect failed (or URL API not available) — show success locally and fall back to in-place update
                setViewMode('data');
                setForm({ title: "", subtitle: "", content: "", image: "", meta_description: "", meta_keyword: "", tags: [], tagInput: '' });
                setFile(null);
                setPreview(null);
                // show popup locally since we couldn't reload
                setSuccess(true);
    ...
    const handleEdit = (a) => {
        setConfirmOpts({
            title: 'Edit artikel',
            message: 'Apakah Anda ingin mengedit Blog ini?',
            variant: 'warning',
            icon: '!',
            shape: 'circle',
            onConfirm: () => {
    ...
    const handleTogglePublish = (id, current) => {
        setConfirmOpts({
            title: current ? 'Unpublish artikel' : 'Publish artikel',
            message: current ? 'Apakah Anda yakin ingin meng-unpublish Blog ini?' : 'Apakah Anda yakin ingin mempublish Blog ini?',
            variant: current ? 'info' : 'success',
            icon: current ? '✗' : '✓',
            shape: 'circle',
            onConfirm: async () => {
                try {
                    await api.put(`/blogs/${id}`, { published: !current });
                    fetchItems();
                    setSuccessMsg(current ? 'Blog berhasil di-unpublish!' : 'Blog berhasil dipublish!');
                    setSuccess(true);
                    setErrorMsg('');
                    setTimeout(() => setSuccess(false), 3000);
                } catch (err) {
                    console.error('toggle publish', err);
                    setErrorMsg('Gagal mengubah status upload');
                } finally {
                    setConfirmOpen(false);
                }
            }
    ...
    const handlePreview = (a) => {
        setPreviewModal(a);
        // keep admin form visible when previewing
    };
        ...
        setConfirmOpts({
            ...
            onConfirm: async () => {
                try {
                    ...
                    fetchItems();
                    setSuccessMsg('Blog berhasil dihapus!');
                    setSuccess(true);
                    setErrorMsg('');
                    setTimeout(() => setSuccess(false), 3000);
                } catch (err) {
    ...
    // Show leave-form confirmation and, if confirmed, reload to reset to data view.
    const confirmLeaveForm = () => {
        setConfirmOpts({
            ...
            icon: '!',
            onConfirm: () => {
                // close modal then reload to the same path to ensure clean state
                setConfirmOpen(false);
    ...
    // Generic helper: if currently in form view, show confirm modal before running the action.
    ...
    return (
        <div className="min-h-screen p-6 bg-gray-50">
            <div className="flex items-center justify-between mb-6">
                <h1 className="text-2xl font-bold text-[#0377FF]">Admin — Blog</h1>
                <div className="flex items-center gap-3">
                    <div className="relative inline-block" ref={dropdownRef}>
                        <button type="button" onClick={() => { if (toggleLocked) return; setOpenDropdown(o => !o); }} className="inline-flex items-center gap-2 bg-[#F0F7FF] border-2 border-[#E3F2FD] rounded-xl px-4 py-2 text-sm text-[#0377FF] shadow-[0_4px_12px_0_rgba(3,119,255,0.06)] focus:outline-none focus:border-[#2196F3] transition-colors w-44 justify-between">
                            <span className="truncate">{viewMode === 'form' ? 'Form Blog' : 'Data Blog'}</span>
                        ...
                        {openDropdown && (
                            <div className="absolute right-0 mt-2 w-full bg-white border-2 border-[#E3F2FD] rounded-xl shadow-lg z-50 overflow-hidden">
                                ...
                                }} className="w-full text-left px-4 py-2 text-[#0377FF] hover:bg-[#F0F7FF]">Form Blog</button>
                                <button type="button" onClick={() => {
                                    if (toggleLocked) return;
                                    setToggleLocked(true); setTimeout(() => setToggleLocked(false), 320);
                                    leaveOrDo(() => { setViewMode('data'); setOpenDropdown(false); setTimeout(() => { try { quill && quill.update && quill.update(); } catch (e) { } }, 90); });
                                }} className="w-full text-left px-4 py-2 text-[#0377FF] hover:bg-[#F0F7FF]">Data Blog</button>
                            </div>
                        )}
                    </div>

                    <button
                        title={viewMode === 'form' ? 'Kembali' : 'Tambah artikel'}
                        onClick={() => {
                            if (toggleLocked) return;
                            setToggleLocked(true); setTimeout(() => setToggleLocked(false), 320);
                            if (viewMode === 'form') {
            ...
            {viewMode === 'form' && (
                <form onSubmit={handleSubmit} className="bg-white border border-blue-200/50 rounded-2xl shadow-lg p-8 md:p-10 max-w-3xl mx-auto mb-8" style={{ boxShadow: '0 8px 32px rgba(33, 150, 243, 0.12)' }}>
                    ...
                    <div className="w-full mb-3">
                        <div
                            className={`w-full bg-[#F0F7FF] border-2 rounded-xl text-[#2196F3] placeholder-[#90CAF9] focus:outline-none focus:border-[#2196F3] transition-all shadow-[0_4px_12px_0_rgba(3,119,255,0.12)] mb-3 relative ${editorFocused ? 'border-[#2196F3] ring-2 ring-[#2196F3]/30' : 'border-[#E3F2FD]'}`}
                            ...
                            <div className="flex-1 overflow-hidden">
                                ...
                                <div className="text-black p-4 md:p-8">
                                    ...
                                    <div className="flex flex-wrap items-center gap-3 text-sm text-gray-600 mb-4">
                                        {/* no view-count in admin preview; show published/created date if present */}
                                    ...
                                    <div className="text-base md:text-lg font-normal text-black leading-relaxed w-full max-w-none text-justify break-words whitespace-pre-wrap article-content" dangerouslySetInnerHTML={{ __html: previewModal.content || '' }} />
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <ConfirmModal
                ...
                variant={confirmOpts.variant}
                icon={confirmOpts.icon}
                shape={confirmOpts.shape}
            />
        </div>
    );
}